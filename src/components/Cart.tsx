...
import MainCart from '../pages/MainCart';
 ...
 const Cart = (props: any) => {
    ...
    let cart = coordinates.map(e => 
        <>
        <Link to={'/weatherApp/build/mainCart'} key={e.id} state={{ name: e.city, id: e.id, lat: e.lat, lon: e.lon, dImg: e.dImg, nImg: e.nImg }}>
            <div
                ...
                <div
                    className="tittleBlock h-14 bg-white hover:bg-blue-500 hover:text-white transition-all gap-2 p-4 flex flex-row items-center">
                    <div className="cityTittle">
                        <span className="cityName flex-grow font-bold text-xl">{e.city}</span>
                    </div>
                    {/* add burger buttons */}
